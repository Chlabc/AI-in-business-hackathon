  ...
  return (
    <div className={colors.credential}>
      <button
        ...
        aria-label={flipped ? `Show ${name}'s employee identity` : `Show ${name}'s coaching summary`}
        aria-describedby={flipped ? backId : frontId}
      />
      <div className={colors.credentialRotor} data-flipped={flipped}>
        {/* Front: identity + portrait. Hidden from a11y while showing the back. */}
        <div
          id={frontId}
          aria-hidden={flipped}
        ...
        {/* Back: coaching summary only — no portrait. */}
        <div
          id={backId}
          aria-hidden={!flipped}
