...
export function PracticeSession({
  ...
  /** Say what's happening in words a rep understands, not the state machine's name for it. */
  const statusLine = scoring
    ? "Scoring your call…"
    : connecting
      ? "Connecting your microphone…"
      : !connected
        ? "Not started — press the button when you're ready"
        : isSpeaking
          ? "The client is talking — let them finish"
          : "Listening — go ahead";
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.4fr_1fr]">
        <section className="surface-card rounded-xl p-5 sm:p-6 lg:p-8">
          <p className="eyebrow">2 · Talk it out</p>
          ...
          {/* One unmistakable control — this is the only thing to press on this page. */}
          <div className="mt-5 flex flex-col items-center gap-3">
            {!connected ? (
              <button
                ...
                className="btn-lift inline-flex h-14 w-full max-w-sm items-center justify-center gap-2.5 rounded-full bg-accent px-8 text-base font-semibold text-accent-fg transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
              >
              ...
              <button
                ...
                onClick={() => void end()}
                disabled={scoring}
                className="btn-lift inline-flex h-14 w-full max-w-sm items-center justify-center rounded-full border-2 border-danger/50 bg-danger-soft px-8 text-base font-semibold text-danger transition hover:opacity-90 disabled:opacity-60"
              >
                {scoring ? "Scoring…" : "I'm done — score me"}
              </button>
            )}
            <p
              aria-live="polite"
              className={`text-sm ${connected && !isSpeaking ? "font-medium text-ok" : "text-muted"}`}
            >
              {statusLine}
            </p>
          </div>
